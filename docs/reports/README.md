...
| 2026-07-17 | [Phase 2 schema review hardening](2026-07-17-phase-2-schema-review-hardening.md) | database / RLS / Storage | Thirteen defects over six rounds: cross-layer invariants, fail-closed authorization and type-contract fixes, the write-side attachment/Storage boundary, then a bucket-idempotency RLS bypass |
| 2026-07-22 | [PR #81 — @supabase/ssr clients + proxy](2026-07-22-pr-81-supabase-ssr-clients-and-proxy.md) | runtime / auth / config / CI / deps | Phase 2 app layer (merged): the three SSR clients + session-refresh proxy, then a 3-pass "10 angles" self-review — 12 findings (7 fixed, 4 rejected-with-reason, 1 in follow-up #82) + 7 high advisories. Theme: failures that don't throw (build-time env, `getClaims()` returned errors, partial cookie writes) |
